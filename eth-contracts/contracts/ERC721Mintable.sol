...
//  TODO's: Create a Pausable contract that inherits from the Ownable contract - check
//  1) create a private '_paused' variable of type bool - check
//  2) create a public setter using the inherited onlyOwner modifier - check
//  3) create an internal constructor that sets the _paused variable to false - check
//  4) create 'whenNotPaused' & 'paused' modifier that throws in the appropriate situation - check
//  5) create a Paused & Unpaused event that emits the address that triggered the event - check
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(msg.sender == ownerOf(tokenId) ||isApprovedForAll(msg.sender, to), "No permission to approve token.");
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(to != address(0), "Minting requires an actual address");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(ownerOf(tokenId) == from, "Only transfer own token");
...
contract rcERC721Token is ERC721Metadata {
